...
python -m PyInstaller --clean --onefile --noconsole --icon="app.ico" --add-data "app.ico;." --name "AutoFischCore" main.py

echo.
echo [2/2] Đang đóng gói AutoFisch.exe (Giao diện khởi động & tự cập nhật)...
python -m PyInstaller --clean --onefile --noconsole --icon="app.ico" --add-data "app.ico;." --name "AutoFisch" launcher.py
